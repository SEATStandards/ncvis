...
/* This file was modified on 2022/06/29 by Paul Ullrich to conform to C++11 standards */
...
#define STACK_ALLOC(var, type, thresh, count) \
	...
	var = (count) <= (thresh) ? var##_stack_ : reinterpret_cast<type *>(calloc(sizeof(type), count));
...
static void *reallocarray_openbsd(void *optr, size_t nmemb, size_t size);
...
sft_loadmem(const void *mem, size_t size)
{
	...
	if (!(font = reinterpret_cast<SFT_Font *>(calloc(1, sizeof *font)))) {
		return NULL;
	}
	font->memory = reinterpret_cast<const uint8_t *>(mem);
...
sft_lmetrics(const SFT *sft, SFT_LMetrics *metrics)
{
	...
	char tag[4] = {'h','h','e','a'};
	if (gettable(sft->font, tag, &hhea) < 0)
		return -1;
...
init_font(SFT_Font *font)
{
	...
	char tag_head[4] = {'h','e','a','d'};
	if (gettable(font, tag_head, &head) < 0)
		return -1;
	...
	char tag_hhea[4] = {'h','h','e','a'};
	if (gettable(font, tag_hhea, &hhea) < 0)
		return -1;
...
init_outline(Outline *outl)
{
	...
	outl->numPoints = 0;
	outl->capPoints = 64;
	if (!(outl->points = reinterpret_cast<Point *>(malloc(outl->capPoints * sizeof *outl->points))))
		return -1;
	outl->numCurves = 0;
	outl->capCurves = 64;
	if (!(outl->curves = reinterpret_cast<Curve *>(malloc(outl->capCurves * sizeof *outl->curves))))
		return -1;
	outl->numLines = 0;
	outl->capLines = 64;
	if (!(outl->lines = reinterpret_cast<Line *>(malloc(outl->capLines * sizeof *outl->lines))))
		return -1;
	return 0;
}
...
grow_points(Outline *outl)
{
	...
	if (!(mem = reallocarray_openbsd(outl->points, cap, sizeof *outl->points)))
		return -1;
	outl->capPoints = (uint_least16_t) cap;
	outl->points    = reinterpret_cast<Point *>(mem);
	return 0;
}
...
grow_curves(Outline *outl)
{
	...
	if (!(mem = reallocarray_openbsd(outl->curves, cap, sizeof *outl->curves)))
		return -1;
	outl->capCurves = (uint_least16_t) cap;
	outl->curves    = reinterpret_cast<Curve *>(mem);
	return 0;
}
...
grow_lines(Outline *outl)
{
	...
	if (!(mem = reallocarray_openbsd(outl->lines, cap, sizeof *outl->lines)))
		return -1;
	outl->capLines = (uint_least16_t) cap;
	outl->lines    = reinterpret_cast<Line *>(mem);
	return 0;
}
...
{
	const uint8_t *bytes = reinterpret_cast<const uint8_t *>(base), *sample;
...
cmap_fmt4(SFT_Font *font, uint_fast32_t table, SFT_UChar charCode, SFT_Glyph *glyph)
{
	...
	segPtr = reinterpret_cast<const uint8_t *>(csearch(key, font->memory + endCodes, segCountX2 / 2, 2, cmpu16));
...
glyph_id(SFT_Font *font, SFT_UChar charCode, SFT_Glyph *glyph)
{
	...
	*glyph = 0;
	...
	char tag_cmap[4] = {'c','m','a','p'};
	if (gettable(font, tag_cmap, &cmap) < 0)
		return -1;
...
hor_metrics(SFT_Font *font, SFT_Glyph glyph, int *advanceWidth, int *leftSideBearing)
{
	...
	char tag_hmtx[4] = {'h','m','t','x'};
	if (gettable(font, tag_hmtx, &hmtx) < 0)
		return -1;
...
outline_offset(SFT_Font *font, SFT_Glyph glyph, uint_fast32_t *offset)
{
	...
	char tag_loca[4] = {'l','o','c','a'};
	if (gettable(font, tag_loca, &loca) < 0)
		return -1;

	char tag_glyf[4] = {'g','l','y','f'};
	if (gettable(font, tag_glyf, &glyf) < 0)
		return -1;
...
render_outline(Outline *outl, double transform[6], SFT_Image image)
{
	...
	post_process(buf, reinterpret_cast<uint8_t *>(image.pixels));
